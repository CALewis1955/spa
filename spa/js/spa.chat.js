...
spa.chat = (function () {
	...
	var
		...
		onTapToggle, onUpdatechat, onListchange, onTapList,
		onLogin, onLogout, onSetchatee, onSubmitMsg
		;
...
*/
	...
	// Arguments : // * position_type - enum('closed', 'open', or 'hidden')
	//  * callback - optional calback to be run at the end
	//    of slider animation.  The callback receives a jQuery collection
	//    representing the slider div as its single argument
	// Action:
	// This method moves the slider into the requested position.
	// If the requested position is the current position, it returns true without 
...
onListchange = function (event) {
	...
	if (! list_html) {
		list_html = String()
			...
			+ 'To chat alone if the fate of all great souls...<br><br>'
			+ 'No one is online'
			+ '</div>';
		clearChat();	
	}	
...
onLogout = function (event, logout_user) {
	...
	//    false if the requested state canot be met
	//  * chat_model - the chat model object provides methods to
	//    interact with our instant messaging
	//  * people_model - the people model object which provides
	//    methods to manage the list of people the model maintains
	//  * slider_* settings.  All these are optional scalars.
	//      See map.Config.settable_map for a full list
	//      Example: slide_open_em is the open height in ems
	// Action       :
	...
	// Arguments 		: 
	//		* $append_ttarget (example: $('#div_id)).
	// 			A jQuery collection that should represent a 
	// 			single DOM container
	// Action       :
	//   Appends the chat slider to the provided container and fills 
	//   it with HTML content.  It then initializes elements, events,
	//   and handlers to provide the user with a chat-room
	//   interface 
